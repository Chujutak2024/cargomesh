...
To reproduce the local integration test, work from the repository root to start Supabase and apply the migration **locally**. Apply [the MCP scenario seed](../../../../supabase/scenarios/mcp-local/seed.sql) and [the V2 QA scenario seed](../../../../supabase/scenarios/v2-road-baseline/seed.sql) to the local database only; the latter requires `psql -v local_only=1`. In one PowerShell terminal from `cargomesh/`, obtain `API_URL`, `ANON_KEY` and `SERVICE_ROLE_KEY` from `supabase status -o json`; set `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `CARGOMESH_MCP_LOCAL_ENABLED=true` and `CARGOMESH_MCP_PROFILE=V1_REGRESSION`, then run `pnpm dev --hostname 127.0.0.1 --port 3100`. In a second terminal set the same Supabase variables, `MCP_LOCAL_BASE_URL=http://localhost:3100`, `CARGOMESH_MCP_TEST_PASSWORD` to the local-only password in `supabase/seed.sql`, and `CARGOMESH_MCP_QA_TEST_PASSWORD` to the local-only password in the V2 QA seed, then run `pnpm test:mcp:local`. Next normalizes the Result Bridge origin to `localhost` in this setup, so use `localhost` as the client origin. Docker must be available for exact-ID cleanup. The test refuses non-loopback app or Supabase URLs. Run `supabase test db` from the repository root for pgTAP. The local test must never target hosted Supabase.
