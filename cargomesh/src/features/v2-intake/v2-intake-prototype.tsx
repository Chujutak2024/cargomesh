      ...
      <div className={styles.frame}>
        <header className={styles.hero}>
          ...
          <div className={styles.heroActions}>
            <Badge tone="preliminary">{t("Prototipo · no transaccional", "Prototype · non-transactional")}</Badge>
            <Button variant="secondary" type="button" onClick={loadExample}><Sparkles size={16} aria-hidden="true" />{t("Cargar ejemplo provisional", "Load provisional example")}</Button>
          </div>
        </header>

        <div className={styles.notice} role="note">
          <Info size={18} aria-hidden="true" />
          <div><strong>{t("Vista de validación sin guardado", "Validation view without saving")}</strong><span>{t("Los cambios permanecen sólo en este navegador mientras la página está abierta. No se crea una solicitud, no se consultan carriers y no se envía una subasta.", "Changes remain only in this browser while the page is open. No request is created, no carriers are queried, and no auction is submitted.")}</span></div>
        </div>
        ...
        <div className={styles.workspace}>
          ...
          <aside className={`${styles.card} ${styles.summary}`} aria-label={t("Resumen en vivo", "Live summary")}>
            ...
            <div className={styles.summaryBody}>
              ...
              <SummaryRow label={t("Ruta y cobertura", "Route & coverage")} value={t("Proveedor cartográfico pendiente · HAC-25", "Map provider pending · HAC-25")} tone={evidence.route.status} status={t("Desconocido", "Unknown")} hint={t("No se infiere distancia, ETA ni disponibilidad.", "Distance, ETA, and availability are not inferred.")} />
...
function FacilityStep({ draft, update, errorFor, t, origin, destination }: FieldHelpers & { origin: ReturnType<typeof findPrototypeFacility>; destination: ReturnType<typeof findPrototypeFacility> }) {
  return <><h3 className={styles.sectionTitle}><MapPinned size={16} aria-hidden="true" />{t("Selecciona sedes de la organización", "Select organization facilities")}</h3><div className={styles.formGrid}><Select label={t("Sede de origen", "Origin facility")} value={draft.originFacilityId} error={errorFor("originFacilityId")} hint={t("Catálogo provisional validado contra V2Facility.", "Provisional catalog validated against V2Facility.")} onChange={(event) => update("originFacilityId", event.target.value)}><option value="">{t("Seleccionar origen", "Select origin")}</option>{PROTOTYPE_FACILITIES.map((facility) => <option value={facility.id} key={facility.id}>{facility.name} · {facility.city}, {facility.countryCode}</option>)}</Select><Select label={t("Sede de destino", "Destination facility")} value={draft.destinationFacilityId} error={errorFor("destinationFacilityId")} hint={t("Debe ser distinta a la sede de origen.", "Must differ from the origin facility.")} onChange={(event) => update("destinationFacilityId", event.target.value)}><option value="">{t("Seleccionar destino", "Select destination")}</option>{PROTOTYPE_FACILITIES.map((facility) => <option value={facility.id} key={facility.id}>{facility.name} · {facility.city}, {facility.countryCode}</option>)}</Select>{origin ? <FacilityCard facility={origin} t={t} /> : null}{destination ? <FacilityCard facility={destination} t={t} /> : null}</div><RoutePreview t={t} /></>;
}

function FacilityCard({ facility, t }: { facility: NonNullable<ReturnType<typeof findPrototypeFacility>>; t: Translate }) {
  return <article className={styles.siteCard}><div className={styles.siteMeta}><Badge tone="preliminary">{t("Provisional", "Provisional")}</Badge><Badge tone="confirmed">V2Facility</Badge></div><strong>{facility.code}</strong><span>{facility.addressLine} · {facility.city}, {facility.countryCode}</span></article>;
}
