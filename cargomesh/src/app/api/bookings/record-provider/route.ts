...
import { BookingBridgeError, record_provider_booking } from "@/features/booking";
...
function errorResponse(error: unknown) {
  ...
  return NextResponse.json({ ok: false, error: { code: "BOOKING_BRIDGE_FAILED", message } }, { status });
}

export async function POST(request: NextRequest) {
  try {
    const data = await record_provider_booking(await request.json(), origin(request));
