...
- **Limitación aceptada de Sprint 1 — Alexa+:** la persistencia `mcp_account_links` está planificada en [HAC-11](https://linear.app/hackatonteamcargomesh/issue/HAC-11/be-1-persistir-account-linking-y-exponer-consulta-mcp-v2-de) (Axel, Sprint 2); por decisión del Tech Lead no se declara operativa en Gate-1.
...
- **Regresión heredada WebMCP V1:** `pnpm test:mcp:local` falla 1/3 y no está en CI; [HAC-26](https://linear.app/hackatonteamcargomesh/issue/HAC-26/gate-1-integrar-y-aceptar-el-cimiento-v2-del-sprint-1) la registra como fallo conocido, no suite aprobada. Axel separará fixtures V1 y una prueba V2 con escenario explícito; QA verificará el resultado.
...
- **Límite del corte:** build y CI QA pasan; el preview local cargó sin nuevos errores de consola al configurar Supabase local. No se declara operativa la autenticación Alexa+ ni verde la base integrada.
...
| [HAC-22 § 6 y § 9](./SPRINT1_ALEXA_SECURITY_AND_BEDROCK.md): vínculo OAuth exacto `(auth_user_id, oauth_client_id)` | `rg -n "mcp_account_links" supabase/migrations cargomesh/src`; `rg -n "blockedAccountLinks" cargomesh/src/server/mcp/auth/user-token.ts`; `pnpm exec tsx --test --test-name-pattern "account linking fails closed" src/server/mcp/auth/user-token.test.ts` desde `cargomesh/` | `BLOQUEADO — limitación conocida del Sprint 1; persistencia planificada en HAC-11 (Sprint 2, Axel). No se declara operativa en Gate-1 por decisión del Tech Lead.` No existe tabla ni repositorio persistente. El repositorio por defecto `blockedAccountLinks` lanza `FORBIDDEN` (falla cerrado); el test con dependencias inyectadas comprueba rechazo de vínculo ausente/revocado/cliente erróneo. Un token Supabase local válido de A recibió `401` en `/mcp` porque el bearer de usuario no está habilitado/configurado; no alcanzó la búsqueda del vínculo. El comentario de `user-token.ts:75–76` atribuye erróneamente la persistencia a HAC-21, cuyo alcance cubre sedes/servicios ROAD; la persistencia y revocación de `mcp_account_links`, con RLS y tests, están asignadas a [HAC-11](https://linear.app/hackatonteamcargomesh/issue/HAC-11/be-1-persistir-account-linking-y-exponer-consulta-mcp-v2-de). Ningún usuario puede completar la autenticación vinculada de Alexa+ hasta que exista. No es un acceso inseguro ni un defecto de seguridad. | Axel ([HAC-11](https://linear.app/hackatonteamcargomesh/issue/HAC-11/be-1-persistir-account-linking-y-exponer-consulta-mcp-v2-de), Sprint 2); observación menor del comentario incorrecto en código de Axel. |
...
`test:mcp:local`. Según la decisión del Tech Lead, es una **regresión heredada
del flujo WebMCP V1**: espera tres carriers fijos y resultados ajenos al
catálogo V2. No se añaden datos V1 al escenario V2 ni se modifica el catálogo
para satisfacerla. Axel separará la regresión V1 con fixtures propios de una
nueva prueba local V2 alimentada por un escenario V2 explícito, que incluirá
el caso sin candidatos. [HAC-26](https://linear.app/hackatonteamcargomesh/issue/HAC-26/gate-1-integrar-y-aceptar-el-cimiento-v2-del-sprint-1)
la registra como **fallo conocido**, no como suite aprobada. QA (Jean Paul)
verificará la separación y sus resultados cuando Axel los entregue.
...
al workflow en HAC-23, porque dejaría en rojo el gate antes de Gate-1. La
separación V1/V2 y la prueba V2 propia preceden cualquier decisión de incluirlo.
...
|---|---|---|---|
| **Limitación conocida aceptada** | Axel ([HAC-11](https://linear.app/hackatonteamcargomesh/issue/HAC-11/be-1-persistir-account-linking-y-exponer-consulta-mcp-v2-de), Sprint 2; vence 2026-09-30) | `mcp_account_links` aún no persiste ni revoca vínculos: autenticación vinculada de usuarios Alexa+ no operativa en Gate-1 por decisión del Tech Lead. No hay escalamiento de privilegios: falla cerrado. | `rg -n "mcp_account_links" supabase/migrations cargomesh/src`; `rg -n "HAC-21 owns|blockedAccountLinks" cargomesh/src/server/mcp/auth/user-token.ts`; contrastar con el DoD de HAC-11. |
| **MENOR — atribución documental** | Axel (HAC-11/HAC-22) | El comentario `user-token.ts:75–76` apunta a HAC-21 en vez de HAC-11 para la persistencia; QA no edita código de Axel. | `rg -n "HAC-21 owns|blockedAccountLinks" cargomesh/src/server/mcp/auth/user-token.ts`; comparar alcance de [HAC-21](https://linear.app/hackatonteamcargomesh/issue/HAC-21/be-2-modelar-y-aplicar-la-base-de-datos-v2-para-sedes-y-servicios-road) y [HAC-11](https://linear.app/hackatonteamcargomesh/issue/HAC-11/be-1-persistir-account-linking-y-exponer-consulta-mcp-v2-de). |
| **MEDIA — regresión V1 conocida** | Axel separa fixtures V1 y nueva prueba V2; Jean Paul verifica; Cristhian registra en HAC-26 | Subtest heredado de worker WebMCP falla 1/3; espera tres carriers fijos y resultados ajenos a V2. Llegó mergeado sin detección porque `test:mcp:local` no está en CI. No se siembran datos V1 en el escenario V2. La prueba V2 explícita deberá cubrir también cero candidatos. | Con entorno del [README MCP](../../cargomesh/src/server/mcp/README.md), `pnpm test:mcp:local` desde `cargomesh/`; `local-integration.test.ts:295`, `started.candidates.length === 0`. Repetido sin escenario V2; [HAC-26](https://linear.app/hackatonteamcargomesh/issue/HAC-26/gate-1-integrar-y-aceptar-el-cimiento-v2-del-sprint-1) lo conserva como fallo conocido, no suite aprobada. |
