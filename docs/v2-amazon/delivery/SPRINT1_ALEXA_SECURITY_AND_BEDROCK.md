...
- Pendiente de implementación, con dueño de Sprint 2: `mcp_account_links` persistente y consulta MCP V2 en HAC-11; elegibilidad/capacidad ROAD compartida en HAC-12. Create/submit V2 queda fuera de esas asignaciones salvo ampliación explícita. La etiqueta de código se corrigió en la rama Gate-1 para expresar capacidades pendientes; HAC-21 no abarcó esos entregables.
...
Consumir el contrato ROAD ya mergeado de HAC-21 sin atribuirle `mcp_account_links`; HAC-11 implementa el repositorio de account links y la consulta MCP V2 sobre el servicio ROAD de HAC-12. Cualquier create/submit V2 requiere alcance/issue explícita antes de habilitarla. Verificar issuer/client en Supabase hosted. Reconsiderar Bedrock únicamente si HAC-17 entrega permisos/cuota y su narración aporta valor medible.
...
- Contratos: `docs/v2-amazon/contracts/ALEXA_MCP_AWS.md`, `ARCHITECTURE.md`, `V1_BOUNDARY_AND_MIGRATION.md`.
- Evidencia V1 preservada: `docs/v1-webmcp/04-execution/REL02_Public_WebMCP_UAT_Evidence.md`.
