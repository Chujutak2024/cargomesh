...
**Vocabulario UML:** `TransportPlan` es el concepto general usado en la narrativa; `TransportPlanCandidate` es la única clase concreta propuesta para representar cada alternativa. No se crea otra entidad `TransportPlan` al elegirla: se conserva el snapshot/identidad de la alternativa y `SelectionDecision` registra cuál se escogió y qué ofertas la respaldan. Esto evita dos fuentes de verdad para tramos, recursos y elegibilidad. Una selección no convierte retrospectivamente una estimación en oferta.
...
- Cada `TransportPlanCandidate` contiene `1..N` tramos y asignaciones de carga a recursos por tramo. Distingue **dos camiones que reparten la carga**, **tracto + remolque** y **vehículo de escolta**, que cumple una función de seguridad pero no aporta capacidad de carga.
...
Una `CarrierOffer` conserva moneda en su valor `Money`, vigencia y desglose de **tarifa de transporte**, recargo de combustible, peajes, manejo, equipo especial/escolta, cargos de cruce/agente, impuestos y descuentos cuando correspondan. Para la demo V2 se aceptan y comparan **solo ofertas USD**; no se convierte PEN ni se crea un campo `currency` independiente que pueda contradecir a `price`. Cada componente indica `included`, `quoted`, `estimated`, `excluded` o `unknown`, fuente y fecha. Se evita contar dos veces combustible o peajes si ya están incluidos en la tarifa. El precio de una gasolinera concreta no es necesario para el MVP ni sustituye la tarifa del carrier.

Cada oferta debe identificar de forma coherente la solicitud, el plan candidato, el carrier emisor, los servicios y los tramos/asignaciones cubiertos. La invitación (`CarrierOpportunity`) y el servicio responsable de cada tramo son las referencias contra las que se valida; repetir IDs en la representación no permite cambiar de carrier o servicio silenciosamente. En el corte ROAD una decisión del shipper selecciona un plan y **una** oferta atribuible. La cardinalidad UML `1..*` deja abierta una selección futura por varios responsables comerciales, pero cada oferta requerirá su propio booking/compromiso y no se venderá como cotización única sin acuerdo explícito.

El booking autorizado no es automáticamente capacidad bloqueada. Para recursos administrados por CargoMesh, la confirmación debe rastrear `CapacityReservation` por ventana y fuente; para capacidad administrada por el carrier, exige confirmación externa verificable. La validación debe impedir solapes y liberar/cancelar holds con idempotencia. Mientras falte cualquiera de esas pruebas, el estado es pendiente o desconocido, nunca «confirmado» por inferencia.
...
El [MVP ratificado](./PRODUCT_SCOPE.md#corte-de-mvp) se limita a ROAD/USD: basta un plan portador verificable con agenda y una oferta atribuible por decisión. Una alternativa de dos unidades solo se presenta como operativa si ambas capacidades, la misma ventana y la cotización están probadas; de lo contrario permanece en diseño o escenario rotulado. La plataforma conserva la taxonomía multimodal sin afirmar ejecución de RAIL/SEA/AIR hasta tener adaptadores, cupos, pruebas y evidencia. Peajes, combustible y frontera entran como componentes citados o `unknown`; automatizar tarifas y trámites completos no es requisito del primer corte.
