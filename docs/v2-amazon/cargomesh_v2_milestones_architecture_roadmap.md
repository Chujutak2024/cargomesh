...
**Estado:** revisión del roadmap cuyos Hitos 0–5 ya se cargaron en Linear; este archivo no los modifica automáticamente.
...
| HITO 1 · Datos, UI y prototipo | Sprint 1 · 19–25 sep (Lima) | Esquema mínimo de sedes/áreas/lanes ROAD con RLS, escenario y pruebas, inventario MCP local, componentes y prototipo, catálogo de datos carrier, AWS/Drive. No stepper ni mapa multimodal live. |
| HITO 2 · MCP y elegibilidad | Sprint 2 · 26 sep–2 oct | Resolver identidad/acceso remoto del MCP y reglas de elegibilidad/capacidad con pruebas. Las HAC-11…16 ya existentes son plan intermedio y se revisarán; **no** se asumen como issues V2 aprobadas aquí. |
...
El alcance de Hitos 2–5 es dirección, no compromiso de branches ni issues futuras. La [propuesta detallada de Sprint 1](./linear_sprint1_v2_rebase_proposal.md) es el único inventario de issues listo para revisión ahora.

## 3. Correcciones necesarias en los hitos ya cargados en Linear

Estas son discrepancias a corregir **después de aprobar la propuesta**; esta revisión no cambia Linear:

1. **Proyecto / HITO 0:** la descripción del proyecto llama al 19 de octubre «Fecha Límite» y exige «160 pgTAP intactos» y Alexa Skill funcional. Cambiar a meta interna; descubrir pruebas vigentes; declarar el camino Alexa+ elegido y su evidencia. HITO 0 tiene casillas `[X]` pero progreso 0%; no declararlo terminado hasta verificar PRs y contratos.
2. **HITO 1:** el plazo del ciclo termina el 25 en Lima, no el 26 como día laborable adicional. Reducir `v2-multimodal-showcase` a un escenario ROAD inicial si la capacidad del equipo lo exige. Quitar «160 tests» fijo y la afirmación de que dorado `#d2a95f` es un requisito del jurado. Añadir la core de Axel para inventariar/aislar el MCP ya mergeado, su rama declarada y el gate de integración. La meta de UI es prototipo validado, no un stepper conectado completo.
3. **HITO 2:** no pedir implementar `/mcp` desde cero ni afirmar que una ASK Skill conectada es obligatoria por las reglas. Primero auditar el transporte local de PR #80; luego probar auth remota y canal elegido. `p95 <700 ms` es una **hipótesis/objetivo interno medible**, nunca una garantía antes de benchmark. Bedrock/SSML se deciden por valor y evidencia, no por obligación del track.
...
- **Alexa+:** una tool y una llamada se consideran live solo con identidad, transporte, código, pruebas y demo de esa integración; el MCP local de PR #80 es un punto de partida.
...
|---|---|
| 1 | Contrato/ADR de red de datos, migración+RLS, README de escenario y pruebas, inventario MCP local/brechas Alexa, tokens/prototipo, catálogo de fuentes carrier, manifiesto de ramas y acta Gate-1; HAC-17/HAC-18 con comprobantes. |
