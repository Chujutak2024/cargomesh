...
| HITO 2 · Elegibilidad ROAD, tools MCP V2 y acceso Alexa+ | Sprint 2 · 26 sep–2 oct | Sobre la base del HITO 1: elegibilidad/capacidad física, servicios compartidos y conexión Alexa+ real si hay acceso, o simulación rotulada; Bedrock se continúa solo si aporta valor y existe cuota. HAC-11…16 son plan intermedio y se revisarán; **no** son compromisos V2 aprobados aquí. |
...
3. **HITO 2:** no pedir implementar `/mcp` desde cero ni afirmar que una ASK Skill conectada es obligatoria por las reglas. El límite seguro de identidad/transporte empezó en Sprint 1; Sprint 2 debe resolver el ownership de `mcp_account_links`, servicios/tools comerciales V2 y elegibilidad, y probar conexión Alexa+ real solo si hay acceso. La [documentación Alexa+](https://developer.amazon.com/docs/alexaplus/add-ons/home.html) advierte acceso de socios selectos. `p95 <700 ms` es **objetivo interno medible**, nunca garantía previa. Bedrock es opcional.
