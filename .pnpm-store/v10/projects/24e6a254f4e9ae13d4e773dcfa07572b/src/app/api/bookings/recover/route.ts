...
import { BookingBridgeError, prepare_booking_recovery } from "@/features/booking";

export const dynamic = "force-dynamic";

export async function POST(request: NextRequest) {
  try {
    const data = await prepare_booking_recovery(await request.json());
    ...
    return NextResponse.json({ ok: false, error: { code: "BOOKING_RECOVERY_FAILED", message } }, { status });
  }
}
