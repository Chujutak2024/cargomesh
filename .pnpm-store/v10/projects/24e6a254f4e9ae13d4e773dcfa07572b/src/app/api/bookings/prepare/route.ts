...
import { BookingBridgeError, prepare_booking } from "@/features/booking";
...
function errorResponse(error: unknown) {
  ...
  return NextResponse.json({ ok: false, error: { code: "BOOKING_PREPARE_FAILED", message } }, { status });
}

export async function POST(request: NextRequest) {
  try {
    const data = await prepare_booking(await request.json());
